...
gApp.factory('BtfordSocket', ['socketFactory', function(socketFactory) {
    // *socketFactory* is a btford.socket-io socket creator
    // NOTE: needs ("bower_components/socket.io/socket.io.js" dependency)

    // For Unit tests, socketFactory with mock socket-io implementation will come from
...
gApp.factory('LocalClockService', function() {
    var DefaultLocalClockService = {
        Now: function() {
            var clientNow = new Date();
            return clientNow.getTime();
        }
    };
    return DefaultLocalClockService;
});
